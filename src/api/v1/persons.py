...
async def film_by_person(
    ...
    films = await person_service.search_by_film_by_person(
        person_id=person_id,
    ...
    return [ShortFilm(id=row.id, title=row.title, imdb_raiting=row.imdb_raiting) for row in films]
...
async def person_search(
    ...
    if not persons:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail="person not found")
    return [Person(uuid=row.uuid, full_name=row.full_name, films=row.films) for row in persons]
